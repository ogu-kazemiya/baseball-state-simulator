...
from ..common.constants import REQUIRED_COLS
...
def get_season_statcast(year: int, columns: list[str] | None = None) -> pd.DataFrame:
    ...
    if os.path.exists(file_path):
        try:
           ...
           validate_columns(df, columns or [], context=f"get_season_statcast({year}) cache")
           raise e
    ...
    validate_columns(df, columns or [], context=f"get_season_statcast({year}) fetch")
